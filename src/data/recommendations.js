...
const getUserAnimeList = async (emailAddress) =>{
    if(!emailAddress){throw 'You must supply your accounts email address'}
    ...
    if(!user.malUsername){throw 'Error: You dont have a linked myanimelist account, please link your account to get recommendations based on MAL profile'}
    const URL = MAL_API_URL + user.malUsername + '/animelist?fields=completed&limit=1000' 
...
export const getUserRecs = async (emailAddress) => {
    ...
    if (updatedInfo.modifiedCount === 0) throw 'Could not update recommendation successfully'

    return { emailAddress: emailAddress, recommendations: updatedRecommendations };
...
const getAnimeInfo = async (animeId) => {
    const URL = 'https://api.myanimelist.net/v2/anime/' + animeId + '?fields=title,'
    const response = await MAL_HANDLER.request(URL, MAL_CLIENT_HEADERS, 'GET')
