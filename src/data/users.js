...
export const loginUser = async (username, password) => {
  if(!username || !password){throw "You must provide both an username and a password"}
  username = username.toLowerCase()
...
export const changeUserInfo = async (username, emailAddress, password) => {
    if(!emailAddress){throw "You must provide your account's email address to modify username/password"}
    if(!username|| !password){throw "You must provide a username and password to modify."}
    username = username.toLowerCase();
    password = validation.passwordValidation(password);
    const hashedPassword = await bcrypt.hash(password, saltRounds);
    const usersCollection = await users();
    const user = await usersCollection.findOne({emailAddress: emailAddress});
    if (user === null){throw 'No user with provided email found.'}
    const updatedUser = {
        username: username,
        emailAddress: emailAddress,
        hashedPassword: hashedPassword
